...
const UserController = {
    ...
    createUser: async (req, res, next) => {
        ...
        if(user){
            res.status(404).send({error: `El usuario ${username} o el email ${email} ya se encuentra en uso`});
            return;
        }
    ...
    autenticarUsuario: async (req, res, next) => {
        ...
        passport.authenticate("local", {sesion: false}, (error, user) =>{
            ...
            const token = jwt.sign(payload, process.env.JWT_SECRET, {algorithm: process.env.JWT_ALGORITHM});
            res.json({token});
        })(req, res, next);
